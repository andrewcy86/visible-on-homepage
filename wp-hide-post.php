...
Plugin URI: 
Description: Controls the posts that appear on the homepage of the consolidated EPA blog.
Version: 1
Author: Andrew Y.
Author URI: 
...
function wphp_query_posts_where($where) {
	...
	// EPA edited to restrict filtering to front page and reversed query. filter posts on one of the three kinds of contexts: front, category, feed
	if( wphp_is_front_page() ) {
		$where .= ' AND wphptbl.post_id IS NOT NULL ';
	}
	//echo "\n<!-- WPHP: ".$where." -->\n";
	return $where;
}
...
function wphp_metabox_post_edit() {
	...
	// EPA removed unnecessary input fields.
?>
    <label for="wplp_post_front" class="selectit"><input type="checkbox" id="wplp_post_front" name="wplp_post_front" value="1"<?php checked($wplp_post_front, 1); ?>/>&nbsp;<?php _e('Show on the front page.', 'wp-hide-post'); ?></label>
